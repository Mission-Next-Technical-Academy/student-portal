...
|---|---|---|---|
| 0 | Lock source of truth: 70/12/82 hours, 42/40 theory/lab, Diploma credential | **DONE** | `docs/specs/CURRICULUM_MAP.md`; `portal/data.js` `program.parents` (all 8 SOC-101.x codes/hours); `bin/curriculum-check.js` asserts totals — passes |
...
| 2 | Form 301 parent mapping on every lesson/lab/assessment | **DONE** | Every item in `portal/data.js` carries `parentAllocations` in minutes; checker enforces allocations sum to item duration; `docs/specs/CURRICULUM_MAP.md` is the exportable mapping report |
| 3 | Reframe Modules 04/08/10, guardrail 05/06 | **DONE** | Exact checklist titles live in `portal/data.js:303,345,360`; `docs/specs/CURRICULUM_ALIGNMENT_ARCHITECTURE.md:82-95` records the mapping; `soc-analyst-module-05.js`/`06.js` content stays at investigation/triage scope — no reverse-engineering or "Threat Hunter" specialization content found |
...
| 9 | Final CIE/launch QA | **Cannot be self-certified** | Requires curriculum-lead, compliance-reviewer, and faculty-reviewer sign-off by design — see `docs/specs/CURRICULUM_ALIGNMENT_ARCHITECTURE.md` §9 "Release boundary" |
...
- The fixed-credit-hour award mechanism — the exact thing described at the
  ...
  matching `docs/specs/CURRICULUM_MAP.md` exactly) and a trigger,
  ...
  `supabase_migration_sync_check` memory. `docs/compliance/COMPLIANCE_DECISIONS_NEEDED.md`'s
...
`docs/compliance/COMPLIANCE_DECISIONS_NEEDED.md` lays out four options and recommends
...
- [ ] Decide evaluator/supervision approach for labs (`docs/compliance/COMPLIANCE_DECISIONS_NEEDED.md` Decision 1)
...
`helpdesk_course_status` memory and `docs/specs/CURRICULUM_ALIGNMENT_ARCHITECTURE.md`
