...
`sprint: 8` — blocked pending the documented compliance decision on the
module-assessment minutes. Sprint 7 is complete: the collapsed shared program
card now shows Start, Continue, or Review Module, and its secondary control
reveals curriculum blocks and labs. Only the blocked Sprint 8 and final QA
Sprint 9 remain.

Note for whoever runs sprint 7: this working tree currently has other,
unrelated uncommitted changes to `portal/app.js` and several `portal/
m360/*` files from a concurrent/earlier session's in-progress work
(multitrack admin M360 progress feature). That's not this project's
work — don't commit it, don't revert it, don't let a sprint 7 agent touch
those parts of app.js. Stage and diff-review narrowly, the way every
sprint so far has.
...
| 6 | Lab 1 hardening pass: verify/complete evidence checkpoints, timeline, disposition+priority+rationale, scored result, coaching against the brief | **done** | `portal/soc-analyst-module-01.js` | Found one real gap: sprint 5b's removal of Lab 1's case-note field had also silently removed Lab 1's only "rationale" artifact (verdict=disposition and priority were still fine). Fixed with a required rationale textarea (40-char min), scored and validated independently of Lab 2. Rebalanced: verdict/25+priority/20+lifecycle/15+action/20+rationale/20=100. Everything else (multi-source evidence, checkpoints/timeline, scored coaching, Lab 1/2 separation) was confirmed already satisfied, no changes needed. Orchestrator fixed one cosmetic leftover (a stale "four-part" form heading from sprint 5b, now "five-part" again) that the agent's own render check didn't catch because it happened to only exercise the Lab-1-locked render state |
| 7 | Program overview UX: primary action (Start/Continue/Review Module) visible on the **collapsed** module card; chevron becomes a secondary "View curriculum blocks and labs" control | **done** (commit `ecd6a3e`) | `portal/app.js` (`moduleCard()`) | Shared component verified through `node bin/portal-check.js 1` across SOC Analyst, IT Support, AI Management, and Electrical Engineering. Draft modules retain their prior non-actionable behavior. |
