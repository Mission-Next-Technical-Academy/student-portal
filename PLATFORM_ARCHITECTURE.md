# MNT Academy — SOC Analyst Program Platform Architecture

Status: **draft v1, architecture only — no implementation started**
Date: 2026-08-17
Companion documents: `MNT_DESIGN_TOKENS.md` (extracted live-site design system),
...
bundle. `github.com/Mission-Next-Technical-Academy` is an empty shell — no repos.
Nothing about the site exists off the deploying machine.

Nothing in §5–§6 can be implemented until one of these happens:

1. The site repo is pushed to the MNT org and cloned here, **or**
2. The source is copied from wherever it currently lives, **or**
3. A decision is made to rebuild the site from the extracted tokens (§`MNT_DESIGN_TOKENS.md`
...
- Simulator fixture data stays fictional. The Mission Next Labs tenant
...
from tokens already on the page (`MNT_DESIGN_TOKENS.md`).
...
| File | Purpose |
|---|---|
| `ui/mnt-session.js` | Reads the Supabase session from the shared origin; exposes `MNT.user`, `MNT.labKey`, `MNT.entitled`. Loads **first**, before `data.js`. |
| `ui/mnt-persistence.js` | Hydrate/flush shim (§7.2). Loads **immediately after** `mnt-session.js`, before any other script touches storage. |
| `ui/mnt-lab-harness.js` | Reads `?lab=<lab_key>` , seeds the scenario, watches for completion conditions, writes `lab_attempts`. Loads **last**, after `workflow-automation.js`. |
...
Key→namespace routing lives in one table in `mnt-persistence.js`:
...
about enrollments — it asks `mnt-session.js`, which checks the session and
...
|---|---|---|
| **0** | Get the marketing site source into the MNT GitHub org and cloned here | — (**blocking, do first**) |
...
2. **Secondary button style** — the site has none. §`MNT_DESIGN_TOKENS.md`
...
- Brand written exactly as **Mission Next Technical Academy Labs** in the
  simulator; the marketing site uses **Mission Next Technical Academy**
