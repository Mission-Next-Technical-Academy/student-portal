...
`sprint: 7` — not started yet. All lesson and lab work is now done (rows
1-6). Read "Sprint 7" row below and launch it — the program-overview
card UX change. This is a SHARED component (`moduleCard()` in
`portal/app.js`) used by all four tracks, not just SOC Analyst — read its
row's blast-radius warning before writing the brief. After sprint 7, only
sprint 8 (blocked) and sprint 9 (final QA) remain.
