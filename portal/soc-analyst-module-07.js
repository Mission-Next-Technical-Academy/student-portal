...
const MODULE_SEVEN_QUIZ_BANKS = [
  ...
  },
  {
    ...
    questions: [
      {
        id: 'm07-q-corr-1',
        prompt: "A phishing message is opened at 10:16 UTC. Network logs show a DNS query at 10:17 UTC resolving auth-renewal.example from the same device. A TLS connection at 10:17:37 connects to the resolved IP. What is the strongest claim?",
        options: [
      ...
      {
        id: 'm07-q-corr-2',
        prompt: "An HTML attachment contains a link to auth-renewal.example. DNS resolves that domain to 203.0.113.88. A TLS session connects to 203.0.113.88 with SNI auth-renewal.example. What does this chain demonstrate?",
        options: [
...
const MODULE_SEVEN_SOURCES_LIST = [
  ...
  {
    title: 'Security+ Certification (SY0-701): Email Security and Threat Investigation',
    org: 'CompTIA',
    url: 'https://www.comptia.org/certifications/security',
    note: 'Official exam objectives covering SOC analyst responsibilities in email analysis, artifact inspection, and evidence-bounded response.',
  },
];
...
function moduleSevenVideoScript() {
  return `<details class="m07-video-script">
    ...
    <div class="m07-script-body">
      ...
      <p><strong>Segment 1 — Reading email authentication signals.</strong> A message's visible From address is presentation. The real sender is authenticated through SPF (envelope validation), DKIM (body signature), and DMARC (alignment). A passing SPF result proves the mailserver sent the message from the envelope domain, but does not validate the displayed From. DMARC failure means the visible identity is not aligned with what the mailserver authenticated. Phishing messages commonly use SPF-passing lookalike domains (northstarr-benefits.example for northstar-benefits.example) to fool both users and simple SPF-only checks.</p>
  ...
  return `<div class="m07-shell">
    ...
    <main class="m07-main">
      ...
      <details class="m07-section-collapsible" ${labOpen ? 'open' : ''}>
        <summary class="m07-section"><div class="m07-section-heading"><span class="m07-section-badge">3</span><div><p class="m07-kicker">Investigation Lab</p><h2 id="m07-lab">Benefits renewal message</h2></div></div></summary>
        <div class="m07-section-body">
