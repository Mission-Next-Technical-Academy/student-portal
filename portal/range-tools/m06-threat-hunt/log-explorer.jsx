// Log Explorer — vendor-neutral log query workspace for Module 6.
//
// This is the same ported Log Explorer tool used by Module 3
// (portal/range-tools/m03-log-explorer/), copied and generalized rather than
// duplicated as a distinct tool — docs/LAB_MIGRATION_MATRIX.md's Module 6
// row calls for reusing M3's adapted SIEM engine, not building a second one.
    ...
    <div style={styles.root}>
      <header style={styles.top}>
        ...
        <div style={styles.topMeta}>
          <span>Module 6 · Mission Next</span>
        </div>
      </header>
...
function LogExplorerConsole({ mode }) {
  const mod = window.M06_LOG_DATASET;
  ...
  React.useEffect(() => {
    if (solvedCount === mod.tasks.length && mod.tasks.length > 0) {
      window.opener?.postMessage({ type: 'm06-log-explorer-complete', mode, solvedCount, total: mod.tasks.length }, location.origin);
    }
