...
function moduleFourLoad(user) {
  ...
  if (!Array.isArray(moduleFourState.feedback)) moduleFourState.feedback = [];
  if (!Array.isArray(moduleFourState.flags)) moduleFourState.flags = [];
  ...
  if (typeof moduleFourState.practiceNotes !== 'string') moduleFourState.practiceNotes = '';
...
function moduleFourGetSections() {
  return [
    ...
    { id: 'guided-lab', title: 'Guided Lab', type: 'lab', isComplete: moduleFourState.practiceComplete, scrollId: 'm04-guided-lab' },
...
function moduleFourGetQuickNavItems() {
  ...
  items.push({
    ...
    title: 'Guided Lab',
    kind: 'lab',
    isComplete: moduleFourState.practiceComplete === true,
...
function moduleFourAssessmentLabPanel() {
  ...
  return `<section class="m04-external-lab" id="m04-assessment-lab-panel">
    ...
    <form id="m04-assessment-form">
      ...
      <div class="m04-actions"><button type="submit" class="m04-primary">${moduleFourState.completed ? 'Resubmit for review' : 'Submit for review'}</button></div>
    </form>
    ${feedbackHtml}
  </section>`;
}
    ...
    <details class="m04-section-collapsible" ${guidedLabOpen ? 'open' : ''}>
      <summary class="m04-section-summary">
        <section class="m04-section m04-lab-section" id="m04-guided-lab" aria-labelledby="m04-guided-lab-title">
          <div class="m04-section-heading"><span>3</span><div><p class="m04-kicker">Practice It · Guided Lab</p><h2 id="m04-guided-lab-title">Active Directory health-check practice</h2></div></div>
        </section>
      </summary>
    ...
    <details class="m04-section-collapsible" ${assessmentLabOpen ? 'open' : ''}>
      <summary class="m04-section-summary">
        <section class="m04-section m04-lab-section" id="m04-assessment-lab" aria-labelledby="m04-assessment-lab-title">
          <div class="m04-section-heading"><span>4</span><div><p class="m04-kicker">Prove It · Assessment Lab</p><h2 id="m04-assessment-lab-title">Independent monitoring and alerting review</h2></div></div>
        </section>
      </summary>
  ...
  return `<div class="m04-shell">
    ...
    <div class="mquick-nav-layout">
      ...
      <section class="m04-hero" aria-labelledby="m04-title">
        <div><p class="m04-kicker">Module 04 · ${formatHandsOnDuration(module.durationMinutes)} · assisted workflow</p><h1 id="m04-title">${esc(module.title)}</h1><p>Work through imported Active Directory health-check and monitoring/alerting labs, then explain what the evidence supports and how you would act on it.</p></div>
        <dl class="m04-status" aria-label="Saved lab status"><div><dt>Guided Lab</dt><dd>${moduleFourState.practiceComplete ? 'Complete' : 'Not started'}</dd></div><div><dt>Assessment Lab</dt><dd id="m04-status">${complete ? 'Complete' : moduleFourState.attempts ? 'In progress' : 'Not started'}</dd></div></dl>
      </section>

      <section class="m04-objective" aria-labelledby="m04-objective-title"><span><i class="ri-focus-2-line" aria-hidden="true"></i></span><div><p class="m04-kicker">One measurable objective</p><h2 id="m04-objective-title">Work through both Active Directory labs and justify a defensible monitoring/alerting recommendation in your assessment write-up.</h2></div></section>
...
function wireModuleFourGuidedLab() {
  ...
  root.addEventListener('input', (event) => {
    if (event.target.matches('[data-m04-practice-notes]')) {
      moduleFourState.practiceNotes = event.target.value;
      moduleFourSave();
    }
  });
  root.addEventListener('click', (event) => {
    if (event.target.closest('[data-m04-practice-complete]')) {
      moduleFourState.practiceComplete = true;
      moduleFourSave();
...
function wireModuleFourAssessmentLab() {
  ...
  root.addEventListener('submit', (event) => {
    if (event.target.id !== 'm04-assessment-form') return;
    event.preventDefault();
    const notes = event.target.querySelector('#m04-assessment-notes')?.value || '';
    moduleFourState.notes = notes;
    if (notes.trim().length < 80) {
      moduleFourState.feedback = ['Write at least 80 characters describing your findings and recommended action before submitting.'];
      moduleFourSave();
    ...
    moduleFourState.feedback = ['Submitted. This write-up has been recorded as your Assessment Lab submission for instructor review.'];
    if (!moduleFourState.flags.includes(MODULE_FOUR_FLAG)) moduleFourState.flags.push(MODULE_FOUR_FLAG);
    if (typeof recordLabAttempt === 'function') {
      recordLabAttempt(moduleFourUser, MODULE_FOUR_CATALOG_LAB_KEY, { state: 'complete', result: { notes } });
    }
