...
function moduleSixLoad(user) {
  ...
  if (typeof moduleSixState.practiceNotes !== 'string') moduleSixState.practiceNotes = '';
...
function moduleSixGetSections() {
  return [
    ...
    { id: 'guided-lab', title: 'Guided Lab', type: 'lab', isComplete: moduleSixState.practiceComplete, scrollId: 'm06-guided-lab' },
    { id: 'assessment-lab', title: 'Assessment Lab', type: 'review', isComplete: moduleSixState.completed, scrollId: 'm06-assessment-lab' },
...
function moduleSixGetQuickNavItems() {
  ...
  items.push({
    ...
    title: 'Guided Lab',
    kind: 'lab',
    isComplete: moduleSixState.practiceComplete === true,
...
const MODULE_SIX_RETURN_TO = encodeURIComponent('/#/program/soc-analyst/module/6');

function moduleSixGuidedLabPanel() {
  return `<section class="m06-external-lab" id="m06-guided-lab-panel">
    <p class="m06-instruction">Work the imported Splunk SIEM DNS log-analysis module below — it walks through spotting C2 beaconing and tunneling behavior in DNS query telemetry. It opens in a new tab. When you're done, note what you found and mark the Guided Lab complete.</p>
    <div class="m06-external-lab-links"><a class="m06-lab-launch" href="imported-labs/mission-next-labs/index.html?returnTo=${MODULE_SIX_RETURN_TO}#/track/splunk/module/mod-1" target="_blank" rel="noopener"><i class="ri-external-link-line" aria-hidden="true"></i> Launch: DNS Log Analysis — C2 Beaconing &amp; Tunneling</a></div>
...
function moduleSixAssessmentLabPanel() {
  ...
  return `<section class="m06-external-lab" id="m06-assessment-lab-panel">
    ...
    <div class="m06-external-lab-links"><a class="m06-lab-launch" href="imported-labs/mission-next-labs/index.html?returnTo=${MODULE_SIX_RETURN_TO}#/track/splunk/module/mod-4" target="_blank" rel="noopener"><i class="ri-external-link-line" aria-hidden="true"></i> Launch: SSH Log Analysis — Brute Force &amp; Credential Stuffing</a></div>
    <form id="m06-assessment-form">
      ...
      <div class="m06-actions"><button type="submit" class="m06-submit">${moduleSixState.completed ? 'Resubmit for review' : 'Submit for review'}</button></div>
    </form>
    ${feedbackHtml}
  </section>`;
}
  ...
  return `<div class="m06-shell">
    ...
    <div class="mquick-nav-layout">
      ...
      <details class="m06-section-collapsible" ${guidedLabOpen ? 'open' : ''}>
        <summary class="m06-section"><div class="m06-section-heading"><span class="m06-section-badge">3</span><div><p class="m06-kicker">Practice It · Guided Lab</p><h2 id="m06-guided-lab">DNS log analysis practice</h2></div></div></summary>
        <div class="m06-section-body">
          <div class="m06-boundary"><i class="ri-shield-check-line" aria-hidden="true"></i><p><strong>Lab boundary:</strong> This lab opens in a separate imported training application in a new tab.</p></div>
          <div id="m06-guided-lab-dynamic">${moduleSixGuidedLabPanel()}</div>
        </div>
      </details>

      <details class="m06-section-collapsible" ${assessmentLabOpen ? 'open' : ''}>
        <summary class="m06-section"><div class="m06-section-heading"><span class="m06-section-badge">4</span><div><p class="m06-kicker">Prove It · Assessment Lab</p><h2 id="m06-assessment-lab">Independent SSH log analysis review</h2></div></div></summary>
        <div class="m06-section-body">
...
function wireModuleSixGuidedLab() {
  ...
  root.addEventListener('input', (event) => {
    if (event.target.matches('[data-m06-practice-notes]')) {
      moduleSixState.practiceNotes = event.target.value;
      moduleSixSave();
    }
  });
  root.addEventListener('click', (event) => {
    if (event.target.closest('[data-m06-practice-complete]')) {
      moduleSixState.practiceComplete = true;
      moduleSixSave();
...
function wireModuleSixAssessmentLab() {
  ...
  root.addEventListener('submit', (event) => {
    if (event.target.id !== 'm06-assessment-form') return;
    event.preventDefault();
    const notes = event.target.querySelector('#m06-assessment-notes')?.value || '';
    moduleSixState.notes = notes;
    if (notes.trim().length < 80) {
      moduleSixState.feedback = ['Write at least 80 characters describing your findings and recommended action before submitting.'];
      moduleSixSave();
    ...
    moduleSixState.feedback = ['Submitted. This write-up has been recorded as your Assessment Lab submission for instructor review.'];
    if (!moduleSixState.flags.includes(MODULE_SIX_FLAG)) moduleSixState.flags.push(MODULE_SIX_FLAG);
    if (typeof recordLabAttempt === 'function') {
      recordLabAttempt(moduleSixUser, MODULE_SIX_CATALOG_LAB_KEY, { state: 'complete', result: { notes } });
    }
