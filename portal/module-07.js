...
const MODULE_SEVEN_PASSING_SCORE = 80;
...
const MODULE_SEVEN_MESSAGES = [
  { id: 'EM-071', time: '10:15', from: 'Benefits Desk <benefits@northstar-people.example>', to: 'acct-63', subject: 'Action required: confirm 2026 benefit election', auth: 'DMARC fail', delivery: 'Delivered · Inbox', trace: 'MT-071', preview: 'Your benefit election will expire today. Open the attached update page to retain coverage.', suspicious: true },
  { id: 'EM-072', time: '09:41', from: 'People Operations <people-ops@mnt-internal.test>', to: 'acct-18', subject: 'August payroll calendar', auth: 'DMARC pass', delivery: 'Delivered · Inbox', trace: 'MT-068', preview: 'The published payroll calendar is available on the internal employee hub.', suspicious: false },
  { id: 'EM-073', time: '09:57', from: 'SOC Briefing <digest@security-news.example>', to: 'acct-52', subject: 'Weekly defensive operations digest', auth: 'DMARC pass', delivery: 'Delivered · Bulk', trace: 'MT-069', preview: 'This week: DNS monitoring, secure mail gateways, and analyst career notes.', suspicious: false },
  { id: 'EM-074', time: '10:11', from: 'Training Team <simulations@mnt-internal.test>', to: 'acct-41', subject: '[EXERCISE] Can you spot this phish?', auth: 'DMARC pass', delivery: 'Delivered · Training', trace: 'MT-070', preview: 'Approved awareness simulation. Report the message using the training button.', suspicious: false },
  { id: 'EM-075', time: '10:24', from: 'Northwind Billing <billing@northwind-supplier.example>', to: 'acct-27', subject: 'Invoice NW-1841 received', auth: 'DMARC pass', delivery: 'Delivered · Inbox', trace: 'MT-074', preview: 'Receipt confirmation for the existing Northwind supplier account. No attachment included.', suspicious: false },
];
...
function moduleSevenLoad(user) {
  ...
  ['reviewedSessions', 'reviewedMessages', 'reviewedEmailTabs', 'selectedEvidence', 'hintsOpened', 'feedback', 'flags'].forEach((key) => {
...
function wireModuleSevenLab() {
  ...
  root.addEventListener('submit', (event) => {
    ...
    if (result.score >= MODULE_SEVEN_PASSING_SCORE) { moduleSevenState.completed = true; if (!moduleSevenState.flags.includes(MODULE_SEVEN_FLAG)) moduleSevenState.flags.push(MODULE_SEVEN_FLAG); moduleSevenMarkCatalogLabs(true); }
