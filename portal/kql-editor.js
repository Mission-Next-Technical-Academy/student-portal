/* ====================================================================
   ...
   Ported from ~/defender-lab/ui/kql-editor.js. Completion is schema-aware:
   pass opts.tables (a function returning { TableName: rows[] }) — the same
   fixtures the console hands to MnKql.evaluate() — so suggestions can never
   drift from what a query will actually return. Items the engine
   (portal/kql-engine.js) understands are tagged "lab"; the rest are real KQL
   kept for study value and tagged "ref".
   ==================================================================== */
...
(function () {
  ...
  const OPERATORS = [
    ...
    { label:'limit',         insert:'limit 10',          detail:'Alias of take', lab:true },
    ...
    { label:'distinct',      insert:'distinct ',         detail:'Unique combinations of columns', lab:true },
    { label:'count',         insert:'count',             detail:'Row count as a single row', lab:true },
    { label:'mv-expand',     insert:'mv-expand ',        detail:'Expand a dynamic array into rows', lab:false },
    { label:'project-rename',insert:'project-rename ',   detail:'Rename a column in place', lab:true },
  ...
  const FUNCTIONS = [
    ...
    { label:'make_set',       insert:'make_set(',          detail:'Set of values in the group', lab:true },
    { label:'make_list',      insert:'make_list(',         detail:'List of values in the group', lab:false },
    { label:'avg',            insert:'avg(',               detail:'Average of a numeric column', lab:true },
    { label:'min',            insert:'min(',               detail:'Minimum in the group', lab:true },
    { label:'max',            insert:'max(',               detail:'Maximum in the group', lab:true },
  ...
  let tableProvider = null;
  function schema() {
    if (schemaCache) return schemaCache;
    const tables = tableProvider ? (tableProvider() || {}) : {};
  ...
  function attachKqlEditor(textarea, opts) {
    ...
    const options = opts || {};
    if (typeof options.tables === 'function') { tableProvider = options.tables; schemaCache = null; }
    ...
    // Make every `el.value = ...` assignment repaint the editor.
  ...
  function savedQueryKey(storageKey) { return (storageKey || 'mn-kql') + '.saved'; }
