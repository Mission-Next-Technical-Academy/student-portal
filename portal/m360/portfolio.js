(() => {
  ...
  function week3(a){
    // Whitelist only sanitized/generalized fields approved for showcase use.
    return [row('Networking learning',a.showcaseInsight||a.generalizedLearning||a.reflection),row('Generalized follow-up',a.showcaseFollowUp||a.generalizedFollowUp),row('Networking achievement',a.showcaseAchievement||a.achievementStatement),list('Sanitized outreach templates',[a.sanitizedTemplate1,a.sanitizedTemplate2,a.sanitizedTemplate3].filter(Boolean))].join('')||'<p class="empty-note">Week 3 is Portfolio Ready. Private networking details remain in the academic record and are intentionally omitted here.</p>';
  }
  function week4(a){const bullets=Array.isArray(a.evidenceBullets)?a.evidenceBullets.filter(x=>x&&x.revised).map(x=>x.revised):[];return [row('Target opportunity',a.targetRole||a.targetOpportunity||a.target),a.resumeUrl?`<div class="portfolio-field-row"><strong>Targeted resume</strong><span>${link(a.resumeUrl,'Open resume')}</span></div>`:'',row('Resume version',a.resumeVersion),list('Revised evidence bullets',bullets),row('Targeting note',a.targetingNote)].join('');}
  ...
  function week6(a){return [a.slidesUrl?`<div class="portfolio-field-row"><strong>Career Spotlight slides</strong><span>${link(a.slidesUrl,'Open slides')}</span></div>`:'',row('Slide 1 · Direction',a.slide1Content),row('Slide 2 · Proof',a.slide2Content),row('Slide 3 · Next',a.slide3Content),row('Presentation status',a.presentationStatus),row('30-day target',a.actionTarget),list('30-day actions',[a.action1,a.action2,a.action3].filter(Boolean)),row('Support / accountability',a.accountability),row('Final reflection',a.reflectionChanged),row('First next step',a.reflectionNextStep)].join('');}
  const renderers={1:week1,2:week2,3:week3,4:week4,5:week5,6:week6};

  function draftSummary(n,d){
    if(!d)return '';
    const labels={1:'Direction',2:'Signal',3:'Connection',4:'Evidence',5:'Voice',6:'Proof'};
    let summary='';
    if(n===1)summary=text(d.direction||d.brandStatement);
    if(n===2)summary=text(d.headline||d.about||d.targetDirection);
    if(n===3)summary=text(d.showcaseInsight||d.reflection||d.generalizedLearning);
    if(n===4)summary=text(d.targetRole||d.targetOpportunity||d.target||d.targetingNote);
    if(n===5)summary=text(d.targetDirection||d.professionalIntroduction||d.improvementNext);
    if(n===6)summary=text(d.slide1Content||d.actionTarget||d.reflectionNextStep);
    return summary?`<article><div class="mini-kicker">Week ${n} · ${labels[n]}</div><p>${esc(summary)}</p><a href="week.html?week=${n}">Continue Week ${n} →</a></article>`:'';
  }
  ...
  async function init(){
    try{
      ...
      if(ctx.isAdmin){$('portfolioNotice').textContent='Admin mode does not display a student portfolio. Use the M360 Review Queue to review student submissions.';return;}
      if(!ctx.eligible){location.replace('../index.html');return;}
      ...
      let ready=0;
      for(let n=1;n<=6;n++){
        const a=accepted(byWeek[n]);
        if(a){ready++;stageReady(n,renderers[n](a));}else stageEmpty(n);
      }
      ...
      $('completionText').textContent=complete?'All six required assignments are accepted, the final average meets the standard, Career Spotlight is complete, and attendance has been staff-verified.':'Final completion requires all six assignments, a final average of 70 or higher, Career Spotlight completion, and staff-confirmed attendance verification.';

      const drafts=[];
      for(let n=1;n<=6;n++){
        const r=byWeek[n],d=draft(r),a=accepted(r);
        if(d&&JSON.stringify(d)!=='{}'&&(!a||JSON.stringify(d)!==JSON.stringify(a))){const h=draftSummary(n,d);if(h)drafts.push(h);}
      }
