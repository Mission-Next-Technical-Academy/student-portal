(() => {
  ...
  function renderPortfolio(rows,progress,{adminMode=false,studentId=''}={}){
    ...
    if(!adminMode){
      ...
      $('portfolioNotice').textContent=`Admin read-only showcase view for Student ${studentId}. Only reviewer-approved portfolio evidence is shown. Private working data, instructor feedback, staff verification fields, raw networking details, and technical-course data are excluded.`;
    }
  }
  ...
  async function init(){
    ...
    try{
      ...
      if(params.get('download')==='1'||params.get('print')==='1')setTimeout(printPortfolio,0);
  ...
  $('printPortfolioBtnBottom').addEventListener('click',()=>{void printPortfolio();});
  init();
})();
