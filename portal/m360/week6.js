(() => {
  ...
  const fieldIds=['slidesUrl','slide1Content','slide2Content','slide3Content','actionTarget','action1','action2','action3','accountability','reflectionChanged','reflectionArtifact','reflectionNextStep'];
  const $=id=>document.getElementById(id);
  const clone=v=>v==null?v:JSON.parse(JSON.stringify(v));
  const esc=v=>String(v??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
  const blank=()=>({slidesUrl:'',slide1Content:'',slide2Content:'',slide3Content:'',actionTarget:'',action1:'',action2:'',action3:'',accountability:'',reflectionChanged:'',reflectionArtifact:'',reflectionNextStep:''});
  ...
  function missing(){
    ...
    const required=[['Slides PDF/link',d.slidesUrl],['Slide 1 content',d.slide1Content],['Slide 2 content',d.slide2Content],['Slide 3 content',d.slide3Content],['30-day target',d.actionTarget],['Action 1',d.action1],['Action 2',d.action2],['Action 3',d.action3],['Support/accountability',d.accountability],['Readiness reflection',d.reflectionChanged],['Artifact reflection',d.reflectionArtifact],['First next step',d.reflectionNextStep]];
  ...
  function renderProof(){
    ...
    $('reviewStatusText').textContent=w.accepted?`Week 6 assignment is reviewer-approved${w.numericScore!=null?` at ${w.numericScore}/100`:''}. Career Spotlight course completion also requires staff-verified presentation completion.`:w.reviewerFeedback||'Continue working, then submit for review.';
  }
