...
How to build a high-fidelity lab on the Boots2Bytes platform. Distilled from the lap-1 (Apache) reference implementation.
...
A lab is a **data object** — not a component. You write a `LabModule` per the schema in `BUILD_PLAN.md` §1.7 and register it on the global `window.B2B_LABS` registry. The platform handles rendering, gating, validation, progress, and the Check-on-Learning drawer for you. Your job is:
...
Every file ends with `Object.assign(window, { ... });` (or namespaced like `window.B2B_LABS`).
...
(function () {
  ...
  const LAP_1_LAB = { /* ... */ };
  Object.assign(window.B2B_LABS = window.B2B_LABS || {}, { 'lap-1': LAP_1_LAB });
})();
```
...
```js
const lap1 = sandbox.window.B2B_LABS['lap-1'];
assert(Array.isArray(lap1.exercises) && lap1.exercises.length === 5, '...');
assert(lap1.checkOnLearning.length >= 5, '...');
const errs = sandbox.window.B2B_LAB_SCHEMA.validateLabShape(lap1);
