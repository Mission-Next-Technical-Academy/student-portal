...
| 4  | Instructor dashboard new fields | done | `engagementRow` panel: step attempts, hints shown, CoL answered/skipped, last seen. Fed by `B2B_PROGRESS_EXT.getAllForUser`. |
...
src/systems/progress.js                           # B2B_PROGRESS_EXT (attempts, CoL, last-seen)
...
```
package.json                  # name + description (boots2bytes)
index.html                    # title + new <script> tags
src/data.js                   # header comment branding
src/app.jsx                   # route resolver checks B2B_LABS first
...
- `B2B_LABS['lap-1']` registered with all required fields
...
The `B2B_LABS` namespace pattern means each agent registers their labs without touching the legacy generic-fixture data. Old shape and new shape coexist; new shape wins on the user route.
