...
(function () {
  ...
  function flattenSteps(lab) {
    return window.B2B_GATING ? window.B2B_GATING.flattenSteps(lab) : (lab.exercises || []).flatMap(e => e.steps || []);
  }
  ...
  function LabPlayer({ lab, user, onBack, track }) {
    ...
    const [completedSet, setCompletedSet] = React.useState(() => {
      // hydrate from B2B_PROGRESS_EXT
      if (!user || !window.B2B_PROGRESS_EXT) return new Set();
      const ext = window.B2B_PROGRESS_EXT.getLabExtended(user.username, lab.id);
    ...
    const [activeStepId, setActiveStepId] = React.useState(() => {
      const next = window.B2B_GATING ? window.B2B_GATING.nextUnlockedStep(lab, completedSet) : null;
    ...
    const [colAnswered, setColAnswered] = React.useState(() => {
      if (!user || !window.B2B_PROGRESS_EXT) return {};
      return window.B2B_PROGRESS_EXT.getCheckpointResponses(user.username, lab.id) || {};
    });
    ...
    function evaluateAllUnlockedSteps(submission, simOverride) {
      ...
      for (const step of flat) {
        ...
        const unlocked = window.B2B_GATING ? window.B2B_GATING.isStepUnlocked(lab, step.id, completedSet) : true;
        if (!unlocked) continue;
        if (step.kind !== 'command' && step.kind !== 'observe' && step.kind !== 'ui') continue;
        const res = window.validateStep(step, sim, submission);
        if (res.ok) {
          if (window.B2B_PROGRESS_EXT && user) window.B2B_PROGRESS_EXT.markStepAttempt(user.username, lab.id, step.id, submission, true);
          recordCorrect(step.id, step.points);
          anyHit = true;
        } else if (step.id === activeStepId) {
          if (window.B2B_PROGRESS_EXT && user) window.B2B_PROGRESS_EXT.markStepAttempt(user.username, lab.id, step.id, submission, false);
        }
      }
      return anyHit;
    }
    ...
    function onAnswerChecked(stepId, submission) {
      ...
      const res = window.validateStep(step, sim, submission);
      if (window.B2B_PROGRESS_EXT && user) {
        window.B2B_PROGRESS_EXT.markStepAttempt(user.username, lab.id, stepId, submission, res.ok);
      }
      if (res.ok) {
    ...
    function onColAnswer(qid, response, passed) {
      ...
      if (window.B2B_PROGRESS_EXT && user) {
        window.B2B_PROGRESS_EXT.markCheckpointResponse(user.username, lab.id, qid, passed, response);
      }
    }
    function onColSkip(qid) {
      setColAnswered(prev => ({ ...prev, [qid]: { passed: false, skipped: true } }));
      if (window.B2B_PROGRESS_EXT && user) {
        window.B2B_PROGRESS_EXT.markCheckpointResponse(user.username, lab.id, qid, false, { skipped: true });
      }
    }
    ...
    function maybeRevealHint(stepId) {
      if (!user || !window.B2B_PROGRESS_EXT) return;
      const att = window.B2B_PROGRESS_EXT.getStepAttempts(user.username, lab.id, stepId);
    ...
    const ShellComponent = window[shellName] || window.LinuxTerminalShell;
  ...
  function StepRow({ step, exerciseIndex, stepIndex, lab, completedSet, isActive, onActivate, hintRevealed, answerRevealed, onAnswerChecked }) {
    ...
    const isLocked = window.B2B_GATING ? !window.B2B_GATING.isStepUnlocked(lab, step.id, completedSet) : false;
