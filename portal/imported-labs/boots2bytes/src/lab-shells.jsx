...
function SplunkLabShell(props) {
  ...
  const defaultFields = ['host', 'source', 'sourcetype', '_time'].map(name => ({ name, count:displayRows.length || mod.logs?.length || 0 }));
  const queryHelpers = [
    `index=missionnext sourcetype=${mod.fields[2] || 'events'}`,
    `search ${mod.fields[1] || mod.fields[0] || 'field'}=`,
    `stats count by ${mod.fields[2] || mod.fields[1] || 'field'}`,
  ];
    ...
    <div style={rs.splunkRoot}>
      ...
      <section style={rs.searchBar}>
        ...
        <div style={rs.splunkSearchRow}>
          <input ref={inputRef} value={query} onChange={event => setQuery(event.target.value)} onKeyDown={handleKeyDown} style={rs.splunkInput} placeholder={`index=missionnext sourcetype=${mod.fields[2] || 'events'} | stats count by ${mod.fields[1] || mod.fields[0] || 'field'}`} />
