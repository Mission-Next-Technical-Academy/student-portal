...
assert(modulePageSource.includes("if (id === 'vm-1') return 'openvas';"), 'Expected ModulePage vm fallback to map vm-1 to OpenVAS');
assert(modulePageSource.includes("if (id === 'vm-5') return 'wsus';"), 'Expected ModulePage vm fallback to map vm-5 to WSUS');
...
const { MISSION_NEXT_LABS, MISSION_NEXT_LAB_SCHEMA, MISSION_NEXT_GATING, MISSION_NEXT_VALIDATOR, MISSION_NEXT_VFS, createVirtualFs, validateStep } = sandbox.window;
assert(MISSION_NEXT_LABS && typeof MISSION_NEXT_LABS === 'object', 'Expected window.MISSION_NEXT_LABS registry to load');
...
const sa5 = MISSION_NEXT_LABS['sa-5'];
const vm1 = MISSION_NEXT_LABS['vm-1'];
const ma1 = MISSION_NEXT_LABS['ma-1'];
const wf1 = MISSION_NEXT_LABS['wf-1'];
...
assert(sa5, 'Expected MISSION_NEXT_LABS["sa-5"] to be registered');
assert(vm1, 'Expected MISSION_NEXT_LABS["vm-1"] to be registered');
assert(ma1, 'Expected MISSION_NEXT_LABS["ma-1"] to be registered');
assert(wf1, 'Expected MISSION_NEXT_LABS["wf-1"] to be registered');
...
const schemaErrs = MISSION_NEXT_LAB_SCHEMA.validateLabShape(lap1);
assert(schemaErrs.length === 0, 'lap-1 schema errors: ' + schemaErrs.join('; '));
const lap2SchemaErrs = MISSION_NEXT_LAB_SCHEMA.validateLabShape(lap2);
...
const sa4SchemaErrs = MISSION_NEXT_LAB_SCHEMA.validateLabShape(sa4);
assert(sa4SchemaErrs.length === 0, 'sa-4 schema errors: ' + sa4SchemaErrs.join('; '));
const sa5SchemaErrs = MISSION_NEXT_LAB_SCHEMA.validateLabShape(sa5);
assert(sa5SchemaErrs.length === 0, 'sa-5 schema errors: ' + sa5SchemaErrs.join('; '));
const vmSchemaErrs = MISSION_NEXT_LAB_SCHEMA.validateLabShape(vm1);
assert(vmSchemaErrs.length === 0, 'vm-1 schema errors: ' + vmSchemaErrs.join('; '));
const maSchemaErrs = MISSION_NEXT_LAB_SCHEMA.validateLabShape(ma1);
assert(maSchemaErrs.length === 0, 'ma-1 schema errors: ' + maSchemaErrs.join('; '));
const wf1SchemaErrs = MISSION_NEXT_LAB_SCHEMA.validateLabShape(wf1);
assert(wf1SchemaErrs.length === 0, 'wf-1 schema errors: ' + wf1SchemaErrs.join('; '));
const wf3SchemaErrs = MISSION_NEXT_LAB_SCHEMA.validateLabShape(wf3);
assert(wf3SchemaErrs.length === 0, 'wf-3 schema errors: ' + wf3SchemaErrs.join('; '));
const wf4SchemaErrs = MISSION_NEXT_LAB_SCHEMA.validateLabShape(wf4);
assert(wf4SchemaErrs.length === 0, 'wf-4 schema errors: ' + wf4SchemaErrs.join('; '));
const wf5SchemaErrs = MISSION_NEXT_LAB_SCHEMA.validateLabShape(wf5);
...
assert(MISSION_NEXT_LABS['vm-2'].environment.shell === 'NessusLabShell', 'Expected vm-2 to use NessusLabShell');
assert(MISSION_NEXT_LABS['vm-3'].environment.shell === 'QualysLabShell', 'Expected vm-3 to use QualysLabShell');
...
assert(MISSION_NEXT_LABS['ad-6'].environment.shell === 'PrometheusLabShell', 'Expected ad-6 to use PrometheusLabShell');
assert(MISSION_NEXT_LABS['ad-7'].environment.shell === 'CactiLabShell', 'Expected ad-7 to use CactiLabShell');
assert(MISSION_NEXT_LABS['wf-1'].environment.shell === 'WindowsEventLogsLabShell', 'Expected wf-1 to use WindowsEventLogsLabShell');
...
assert(MISSION_NEXT_GATING.isStepUnlocked(lap1, firstStep.id, completed), 'Expected first step to be unlocked initially');
assert(!MISSION_NEXT_GATING.isStepUnlocked(lap1, secondStep.id, completed), 'Expected second step to be locked before first completes');
completed.add(firstStep.id);
assert(MISSION_NEXT_GATING.isStepUnlocked(lap1, secondStep.id, completed), 'Expected second step to unlock after first completes');
...
const lap2Step = MISSION_NEXT_GATING.flattenSteps(lap2).find(step => step.id === 'lap-2.ex4.s3');
...
const lap4Flat = MISSION_NEXT_GATING.flattenSteps(lap4);
