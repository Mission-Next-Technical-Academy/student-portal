...
(function () {
  ...
  function provePanel() {
    ...
    return `<div class="m02e-prove-panel" id="m02e-prove-panel"><p class="m02e-label">ANALYST DETERMINATION</p><h3>Identify the policy violation</h3><p class="m02e-muted">Select an activity in Access Activity above, investigate its linked identity, device, resource, and policy records, then submit your determination. Not every denied event is malicious, and not every allowed event is correct.</p><dl class="m02e-fields m02e-prove-selection"><div class="m02e-field"><dt>Selected event</dt><dd>${esc(selectedLabel)}</dd></div></dl><div class="m02e-decisions">${['POLICY VIOLATION', 'NO VIOLATION'].map((d) => `<button class="${p.decision === d ? 'is-selected' : ''}" data-m02e-prove-decision="${d}" ${p.submitted ? 'disabled' : ''}>${d}</button>`).join('')}</div><h4>Evidence referenced</h4>${PROVE_EVIDENCE.map((item) => `<label><input type="checkbox" data-m02e-prove-evidence value="${esc(item)}" ${p.evidence.includes(item) ? 'checked' : ''} ${p.submitted ? 'disabled' : ''}> ${esc(item)}</label>`).join('')}<label class="m02e-rationale">Analyst note<textarea data-m02e-prove-note rows="3" maxlength="600" placeholder="State the policy mismatch and recommended correction." ${p.submitted ? 'disabled' : ''}>${esc(p.note)}</textarea></label><button class="m02e-primary" type="button" data-m02e-prove-submit ${p.submitted ? 'disabled' : ''}>${p.submitted ? 'Submitted for review' : 'Submit determination'}</button>${p.feedback ? `<div class="m02e-feedback">${esc(p.feedback)}</div>` : ''}</div>`;
  }
  ...
  function wire() {
    ...
    root.onclick = (ev) => {
      ...
      if (button.dataset.m02eProveDecision) { if (!state.prove.submitted) { state.prove.decision = button.dataset.m02eProveDecision; save(); renderScope('prove'); } return; }
    ...
    root.onchange = (ev) => {
      ...
      if (t.matches('[data-m02e-prove-evidence]')) { state.prove.evidence = t.checked ? [...new Set([...state.prove.evidence, t.value])] : state.prove.evidence.filter((x) => x !== t.value); save(); return; }
    };
    root.oninput = (ev) => {
