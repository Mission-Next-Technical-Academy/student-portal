...
function viewModuleEleven(user, program) {
  ...
  return `<div class="m11-shell">${moduleTopbar(user, program)}<main class="m11-main"><section class="m11-hero"><div><p class="m11-kicker">Week 6 · Module 11 · ${formatInstructionalMinutes(module.durationMinutes)}</p><h1>${esc(module.title)}</h1><p>Turn operating signals and technical evidence into decisions that analysts, incident owners, and leaders can act on.</p></div><dl><div><dt>Labs complete</dt><dd id="m11-complete-count">${completeCount}/${module.labs}</dd></div><div><dt>Difficulty</dt><dd>Independent</dd></div><div><dt>Pass mark</dt><dd>${MODULE_ELEVEN_PASSING_SCORE}% each</dd></div></dl></section><section class="m11-practice-note"><i class="ri-compass-3-line" aria-hidden="true"></i><div><p class="m11-kicker">Independent practice</p><h2>Read the objective and dataset, then choose your own working order.</h2><p>No prescribed sequence or pre-submission hints are provided. Scoring feedback and a reference model appear after you submit.</p></div></section><div id="m11-lab-dynamic">${moduleElevenDynamic()}</div></main></div>`;
}
