...
| D5 | The real multi-day case: evidence chain, tasks, weighted scoring, consequence branching, **no live score shown to the student** | `portal/soc-analyst-module-01.js` (full rewrite of the lab section), `portal/data.js`, new migration | **Judgment-heavy — do this myself with close review, not blind delegation. The core of this whole plan.** | **Shipped, 2026-09-17** — fixed Northstar Finance case across identity, endpoint, proxy, and user-callback evidence; reviewable evidence records, three Day 1–3 decision points, and branch-visible consequence of containment choice. State now calls D3’s cross-device case beacon. Submission shows confirmation only; internal rubric/answer/critical-error data goes to the existing grading queue. A critical containment error cannot satisfy verified module completion (migration `20260917120000_module_one_case_critical_error_gate.sql`). |
