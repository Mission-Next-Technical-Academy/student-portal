...
- Project folder: `/home/alex/Mission_Next_Technical_Academy_SOC_Analyst_course`
...
- Renamed the new folder to:
  `Mission_Next_Technical_Academy_SOC_Analyst_course`.
...
Working directory: /home/alex/Mission_Next_Technical_Academy_SOC_Analyst_course/ui
...
- Created a dedicated desktop launcher:
  `/home/alex/.local/share/applications/mission-next-soc-analyst-course.desktop`.
- Pinned `mission-next-soc-analyst-course.desktop` to the GNOME dash.
- Configured it to run this course's `bin/launch.sh`.
- Clicking the icon starts the server when needed and opens the course URL.

### 4. Mission Next logo integration
...
- Project/web asset: `ui/assets/mission-next-logo.png`
- Installed dash icon:
  `/home/alex/.local/share/icons/mission-next-soc-analyst-course.png`
...
- Standardized the fictional tenant as **Mission Next Labs** and migrated its
  identities to the reserved `missionnextlabs.example` domain in source
...
- Replaced the temporary generic presentation label with the confirmed
  **Mission Next Labs** identity.
- Normalized tenant email and directory domains to
  `missionnextlabs.example`.
- Updated lowercase resource identifiers to the stable `missionnextlabs`
  token and the directory account domain to `MISSIONNEXTLABS`.
...
- Verified the final header as `Mission Next Labs · Tenant`, checked the
