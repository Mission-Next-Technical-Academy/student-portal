...
|---|---|---|---|
| 0.1 | Locate the `mntacademy.com` Vite source and push it to `github.com/Mission-Next-Technical-Academy` | Alex | Repo exists, clones, `npm run dev` serves the current site |
...
- **32A `ui/mnt-session.js`:** Supabase session read, `MNT.user`, `MNT.labKey`,
  `MNT.entitled`, sign-out hook. Loads before `data.js`.
- **32B `ui/mnt-persistence.js`:** hydrate/run/flush shim per §7.2. Storage
...
- **41C `ui/mnt-lab-harness.js`:** seeds scenario state, watches completion
